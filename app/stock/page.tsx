...
export default function StockPage() {
  ...
  const stock = useMemo(
    () =>
      items.map((item) => {
        const rows = movements.filter((m) => m.itemId === item.itemId);
        const qty = rows.reduce((s, m) => s + n(m.qtyIn) - n(m.qtyOut), 0);
        const value = rows.reduce((s, m) => s + (n(m.qtyIn) ? n(m.value) : -n(m.value)), 0);
  ...
  async function post(
    ...
    setMessage(`${action} saved: ${b.row.itemId || b.row.movementId}`);
    await load();
  }
  ...
  return (
    <>
      <h2>Items & Stock</h2>
      <p className="small">Stock write permission controls item and movement creation.</p>
      {message && (
      ...
      <section className="panel table-wrap">
        ...
        <table className="data-table">
          <thead><tr><th>Item</th><th>Type</th><th>Qty</th><th>Book Value</th></tr></thead>
          <tbody>
            {stock.map((r) => <tr key={r.itemId}><td>{r.itemCode} — {r.itemName}</td><td>{r.itemType}</td><td>{r.qty}</td><td>{money(r.value)}</td></tr>)}
          </tbody>
        </table>
      </section>
      ...
      <section className="panel table-wrap">
        ...
        <table className="data-table">
          <thead><tr><th>Date</th><th>Movement</th><th>Item</th><th>Project</th><th>Qty In</th><th>Qty Out</th><th>Value</th><th>Source</th></tr></thead>
          <tbody>
            {[...movements].reverse().map((r) => <tr key={r.movementId}><td>{r.movementDate}</td><td>{r.movementType}</td><td>{r.itemId}</td><td>{r.projectId || "—"}</td><td>{r.qtyIn || 0}</td><td>{r.qtyOut || 0}</td><td>{money(r.value)}</td><td>{r.sourceDocumentId || "—"}</td></tr>)}
          </tbody>
        </table>
      </section>
    </>
