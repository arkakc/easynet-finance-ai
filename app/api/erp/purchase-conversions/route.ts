...
async function supplierQuoteToPurchaseOrder(supplierQuoteId:string){
 ...
 return {createdId:poId,documentNumber:poNumber,status:"created",createdType:"purchaseOrder"};
}
...
async function purchaseOrderToSupplierInvoice(purchaseOrderId:string){
 ...
 const lines=await findRecords<any>("POLines",{poId:purchaseOrderId},500);if(!lines.rows.length)throw new Error("Purchase Order has no lines");
