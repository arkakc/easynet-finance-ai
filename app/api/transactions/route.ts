...
async function createCommercial(type: "quote" | "purchaseOrder" | "invoice" | "supplierBill", raw: unknown) {
  ...
  if (type === "purchaseOrder") {
    ...
    await appendRecord("PurchaseOrders", {
      ...
      poDate: parsed.documentDate, netAmount: t.net, gstAmount: t.gst,
      totalAmount: t.total, status: "DRAFT", sourceDocumentId: "",
    }, "transaction-ui");
  ...
  if (type === "invoice") {
    ...
    await appendRecord("Invoices", {
      ...
      netAmount: t.net, gstAmount: t.gst, totalAmount: t.total,
      paidAmount: 0, outstandingAmount: t.total, status: "DRAFT",
      sourceDocumentId: "", journalId: "",
    }, "transaction-ui");
    await batchAppend("InvoiceLines", t.lines.map((line) => ({
      invoiceLineId: `${invoiceId}-${String(line.lineNo).padStart(3, "0")}`, invoiceId,
      ...line, revenueAccountId: parsed.accountId || "ACC-4100",
    })), "transaction-ui");
  ...
  await batchAppend("SupplierBillLines", t.lines.map((line) => ({
    billLineId: `${billId}-${String(line.lineNo).padStart(3, "0")}`, billId,
    ...line, costAccountId: parsed.accountId || "ACC-5100",
  })), "transaction-ui");
...
async function createPayment(raw: unknown) {
  ...
  await appendRecord("Payments", {
    paymentId, paymentNumber, ...parsed, sourceDocumentId: "", journalId: "", status: "DRAFT",
  }, "transaction-ui");
  return { type: "payment", recordId: paymentId, documentNumber: paymentNumber, status: "DRAFT" };
}
...
async function createExpense(raw: unknown) {
  ...
  await appendRecord("Expenses", {
    expenseId, expenseNumber, ...parsed, totalAmount,
    sourceDocumentId: "", journalId: "", status: "DRAFT",
  }, "transaction-ui");
...
async function postRecord(raw: unknown) {
  ...
  if (parsed.recordType === "invoice") {
    const result = await findRecords<any>("Invoices", { invoiceId: parsed.recordId }, 1);
    const row = result.rows[0];
    if (!row) throw new Error("Invoice not found");
    if (row.status === "POSTED") return { recordType: parsed.recordType, recordId: parsed.recordId, status: "already-posted", journalId: row.journalId };
    if (Number(row.gstAmount || 0) > 0 && (await gstStatus()) !== "VERIFIED") throw new Error("GST status is UNVERIFIED. Verify GST registration before posting GST-bearing invoices.");
    const lineResult = await findRecords<any>("InvoiceLines", { invoiceId: parsed.recordId }, 500);
    const revenueAccountId = lineResult.rows[0]?.revenueAccountId || "ACC-4100";
    const journal = await postJournal({
      postingDate: String(row.invoiceDate), documentType: "SALES_INVOICE", documentId: row.invoiceId,
      documentNumber: row.invoiceNumber, reference: `Sales invoice ${row.invoiceNumber}`,
      projectId: row.projectId, lines: salesInvoicePosting({ total: Number(row.totalAmount), net: Number(row.netAmount), gst: Number(row.gstAmount), customerId: row.customerId, projectId: row.projectId, revenueAccountId }),
    });
    await updateRecord("Invoices", "invoiceId", row.invoiceId, { status: "POSTED", journalId: journal.journalId }, "finance-controller");
    return { recordType: parsed.recordType, recordId: row.invoiceId, status: "POSTED", journalId: journal.journalId };
  }

  if (parsed.recordType === "supplierBill") {
    const result = await findRecords<any>("SupplierBills", { billId: parsed.recordId }, 1);
    const row = result.rows[0];
    ...
    const lineResult = await findRecords<any>("SupplierBillLines", { billId: parsed.recordId }, 500);
    const costAccountId = lineResult.rows[0]?.costAccountId || "ACC-5100";
    const journal = await postJournal({
      postingDate: String(row.billDate), documentType: "SUPPLIER_BILL", documentId: row.billId,
      documentNumber: row.billNumber, reference: `Supplier bill ${row.billNumber}`,
      projectId: row.projectId, lines: supplierBillPosting({ total: Number(row.totalAmount), net: Number(row.netAmount), gst: Number(row.gstAmount), supplierId: row.supplierId, projectId: row.projectId, costAccountId }),
    });
    await updateRecord("SupplierBills", "billId", row.billId, { status: "POSTED", journalId: journal.journalId }, "finance-controller");
    return { recordType: parsed.recordType, recordId: row.billId, status: "POSTED", journalId: journal.journalId };
  }

  if (parsed.recordType === "payment") {
    const result = await findRecords<any>("Payments", { paymentId: parsed.recordId }, 1);
    const row = result.rows[0];
    if (!row) throw new Error("Payment not found");
    if (row.status === "POSTED") return { recordType: parsed.recordType, recordId: parsed.recordId, status: "already-posted", journalId: row.journalId };
    const receive = row.paymentType === "RECEIVE";
    const lines = receive
      ? receiptPosting({ amount: Number(row.amount), customerId: row.partyId, projectId: row.projectId, cashBankAccountId: row.cashBankAccountId })
      : supplierPaymentPosting({ amount: Number(row.amount), supplierId: row.partyId, projectId: row.projectId, cashBankAccountId: row.cashBankAccountId });
    const journal = await postJournal({
      postingDate: String(row.paymentDate), documentType: receive ? "CUSTOMER_RECEIPT" : "SUPPLIER_PAYMENT",
    ...
    if (row.againstDocumentId) {
      if (receive) {
        ...
        if (inv.rows[0]) {
          const paid = round2(Number(inv.rows[0].paidAmount || 0) + Number(row.amount));
          const outstanding = Math.max(0, round2(Number(inv.rows[0].totalAmount || 0) - paid));
          await updateRecord("Invoices", "invoiceId", row.againstDocumentId, { paidAmount: paid, outstandingAmount: outstanding, status: outstanding === 0 ? "PAID" : "POSTED" }, "finance-controller");
        }
      } else {
        const bill = await findRecords<any>("SupplierBills", { billId: row.againstDocumentId }, 1);
        if (bill.rows[0]) {
          const paid = round2(Number(bill.rows[0].paidAmount || 0) + Number(row.amount));
          const outstanding = Math.max(0, round2(Number(bill.rows[0].totalAmount || 0) - paid));
          await updateRecord("SupplierBills", "billId", row.againstDocumentId, { paidAmount: paid, outstandingAmount: outstanding, status: outstanding === 0 ? "PAID" : "POSTED" }, "finance-controller");
        }
      }
    }
    return { recordType: parsed.recordType, recordId: row.paymentId, status: "POSTED", journalId: journal.journalId };
  }

  const result = await findRecords<any>("Expenses", { expenseId: parsed.recordId }, 1);
  const row = result.rows[0];
  ...
  const journal = await postJournal({
    ...
    lines: expensePosting({ total: Number(row.totalAmount), net: Number(row.netAmount), gst: Number(row.gstAmount), supplierId: row.supplierId, projectId: row.projectId, expenseAccountId: row.expenseAccountId, cashBankAccountId: row.cashBankAccountId }),
  });
