...
const CONFIG: Record<string, { table: string; idField: string; numberField: string; lineTable?: string; lineIdField?: string; permission: Permission; title: string }> = {
  quote: { table: "Quotes", idField: "quoteId", numberField: "quoteNumber", lineTable: "QuoteLines", lineIdField: "quoteId", permission: "sales.read", title: "Quotation" },
  ...
  supplierBill: { table: "SupplierBills", idField: "billId", numberField: "billNumber", lineTable: "SupplierBillLines", lineIdField: "billId", permission: "purchase.read", title: "Supplier Bill" },
  payment: { table: "Payments", idField: "paymentId", numberField: "paymentNumber", permission: "accounts.read", title: "Payment / Receipt" },
  expense: { table: "Expenses", idField: "expenseId", numberField: "expenseNumber", permission: "accounts.read", title: "Expense" },
};

const labels: Record<string, string> = {
  customerId: "Customer", supplierId: "Supplier", projectId: "Project", quoteDate: "Date", invoiceDate: "Date", poDate: "Date", billDate: "Date", paymentDate: "Date", expenseDate: "Date",
  dueDate: "Due Date", expiryDate: "Expiry Date", netAmount: "Net Amount", gstAmount: "GST", totalAmount: "Total", paidAmount: "Paid", outstandingAmount: "Outstanding", status: "Status", reference: "Reference", paymentMethod: "Payment Method", description: "Description", journalId: "Journal", sourceDocumentId: "Source Document",
};
...
function display(key: string, value: unknown) {
  if (moneyFields.has(key)) return `K${Number(value || 0).toFixed(2)}`;
  return String(value ?? "");
}
...
export default async function TransactionDocumentPage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  const number = record[config.numberField] || id;
  const hidden = new Set([config.idField, config.numberField, "createdAt", "updatedAt"]);
  ...
  return <div className="document-page">
    <div className="document-toolbar no-print"><Link href="/transactions">← Transactions</Link><div className="row-actions"><PrintButton /></div></div>
    <section className="document-sheet">
      <header className="document-header"><div><div className="eyebrow">EASYNET IT SOLUTIONS LIMITED</div><h1>{config.title}</h1><div className="document-number">{number}</div></div><div className={`status-pill status-${String(record.status || "draft").toLowerCase()}`}>{record.status || "DRAFT"}</div></header>
    ...
    <DocumentConversionActions type={type} id={id} status={String(record.status || "")} />
  </div>;
}
