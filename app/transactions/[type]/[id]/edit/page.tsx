...
import DraftDocumentEditor from "@/app/components/draft-document-editor";
...
export default async function EditDraftDocumentPage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  if (!record) notFound();
  if (String(record.status || "DRAFT").toUpperCase() !== "DRAFT") notFound();
  if (String(record.journalId || "").trim()) notFound();
  if (Number(record.paidAmount || 0) > 0.0001) notFound();

  if (type === "payment") {
    if (String(record.partyType || "") === "Supplier") await requirePermission("purchase.write");
    else await requirePermission("sales.write");
  }

  const number = String(record[config.numberField] || id);
  const isSupplierQuotation = type === "purchaseOrder" && number.startsWith("SUPQ-");

  return <DraftDocumentEditor
    type={type as "quote" | "purchaseOrder" | "supplierBill" | "payment" | "expense"}
    id={id}
    number={number}
    record={record}
