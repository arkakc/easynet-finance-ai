...
export default function TransactionsPage() {
  ...
  const approvedPurchaseOrders = useMemo(() => tx.purchaseOrders.filter((r: any) => {
    ...
    return rowStatus === "APPROVED" && !no.startsWith("SUPQ-") && !billedPoIds.has(String(r.poId));
  ...
  function setLine(i: number, field: keyof DraftLine, value: string) { setLines(current => current.map((line, index) => index === i ? { ...line, [field]: value } : line)); }
  function addLine() { setLines(current => [...current, { description: "", qty: "1", uom: "Each", rate: "0" }]); }
  function removeLine(i: number) { setLines(current => current.length === 1 ? current : current.filter((_, index) => index !== i)); }
  ...
  return <>
    ...
    {sectionMode === "create" && commercial && <form className="panel" onSubmit={submitCommercial}>
      ...
      <h4>Lines</h4>
      <div className="table-wrap"><table className="data-table" style={{ minWidth: 920 }}><thead><tr><th style={{ width: "42%" }}>Item Description</th><th>QTY</th><th>UOM</th><th>Unit Price</th><th>Total Price</th><th></th></tr></thead><tbody>{lines.map((line, index) => <tr key={index}><td><input value={line.description} onChange={event => setLine(index, "description", event.target.value)} required /></td><td><input type="number" min="0.0001" step="0.0001" value={line.qty} onChange={event => setLine(index, "qty", event.target.value)} /></td><td><input value={line.uom} onChange={event => setLine(index, "uom", event.target.value)} /></td><td><input type="number" min="0" step="0.01" value={line.rate} onChange={event => setLine(index, "rate", event.target.value)} /></td><td><strong>{money((Number(line.qty) || 0) * (Number(line.rate) || 0))}</strong></td><td><button type="button" className="secondary" onClick={() => removeLine(index)}>Remove</button></td></tr>)}</tbody></table></div>
