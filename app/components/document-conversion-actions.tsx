...
export default function DocumentConversionActions({type,id,status,documentNumber}:{type:string;id:string;status:string;documentNumber?:string}){
  ...
  function openPurchaseReceipt(){ router.push(`/stock?mode=movement&sourcePo=${encodeURIComponent(id)}`); }
  ...
  async function convert(){
    ...
    try{
      ...
      if(canInvoicePayment){
        ...
        setPaymentContext(b.invoice);return;
      }
      if(canSupplierQuote){
        ...
        const b=await r.json();if(!r.ok||!b.ok)throw new Error(b.error||"Conversion failed");
  ...
  async function createPaymentDraft(e:FormEvent<HTMLFormElement>){
    e.preventDefault();if(!paymentContext)return;setBusy(true);setMessage("");
    try{
      ...
      const r=await fetch("/api/erp/transactions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"createPayment",payload:{paymentNumber:"",paymentType:"RECEIVE",partyType:"Customer",partyId:paymentContext.customerId,projectId:paymentContext.projectId||"",paymentDate:f.get("paymentDate"),amount,paymentMethod:f.get("paymentMethod"),cashBankAccountId:f.get("cashBankAccountId"),reference:f.get("reference")||"",againstDocumentType:"Sales Invoice",againstDocumentId:paymentContext.invoiceId}})});
      const b=await r.json();if(!r.ok||!b.ok)throw new Error(b.error||"Payment draft creation failed");
      router.push(`/transactions/payment/${b.result.recordId}`);router.refresh();
  ...
  return <div className="conversion-box no-print">
    ...
    {!paymentContext&&canPo&&<div className="button-row"><button type="button" className="secondary" disabled={busy} onClick={openPurchaseReceipt}>Create Purchase Receipt / Goods Receipt</button><button type="button" disabled={busy} onClick={convert}>{busy?"Loading…":"Create Supplier Invoice"}</button></div>}
    ...
    {paymentContext&&<form onSubmit={createPaymentDraft} className="form-grid" style={{marginTop:16}}>
      <label>Sales Invoice<input value={paymentContext.invoiceNumber||paymentContext.invoiceId} readOnly/></label><label>Customer<input value={paymentContext.customerId} readOnly/></label><label>Outstanding<input value={`K${Number(paymentContext.outstandingAmount||0).toFixed(2)}`} readOnly/></label><label>Payment Date<input name="paymentDate" type="date" defaultValue={today()} required/></label><label>Amount<input name="amount" type="number" min="0.01" max={paymentContext.outstandingAmount} step="0.01" defaultValue={paymentContext.outstandingAmount} required/></label><label>Payment Method<select name="paymentMethod" defaultValue="" required><option value="">Select payment method</option><option>Cash</option><option>Bank Transfer</option><option>Card</option><option>Cheque</option></select></label><label>Cash / Bank Account<input name="cashBankAccountId" placeholder="e.g. ACC-1120" required/></label><label>Reference<input name="reference" placeholder="Bank / receipt reference"/></label><div className="form-wide button-row"><button type="button" className="secondary" disabled={busy} onClick={()=>setPaymentContext(null)}>Cancel</button><button type="submit" disabled={busy}>{busy?"Saving Draft…":"Save Payment Draft"}</button></div>
    </form>}
