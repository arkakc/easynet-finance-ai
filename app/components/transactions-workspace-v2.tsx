...
function resolveParty(options: any[], input: string) {
  ...
  return options.find((row) => partyDisplay(row).toLowerCase() === query)
    ...
    || options.find((row) => partyName(row).toLowerCase() === query)
    || null;
}

export default function TransactionsWorkspaceV2() {
  ...
  async function loadItems() {
    ...
    try {
      ...
      if (!response.ok || !body.ok) throw new Error(body.error || "Item Master load failed");
      setItems(body.items || []);
      setItemsLoaded(true);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Item Master load failed");
    }
  }
  ...
  async function loadTransactions(force = false) {
    ...
    try {
      ...
      const next = {
        quotes: body.quotes || [],
        supplierQuotes: body.supplierQuotes || [],
        purchaseOrders: body.purchaseOrders || [],
        invoices: body.invoices || [],
        supplierBills: body.supplierBills || [],
  ...
  async function loadNextDocumentNo(currentTab: Tab) {
    ...
    try {
      ...
      const response = await fetch(`/api/erp/transactions?${params}`, { cache: "no-store" });
      const body = await response.json();
  ...
  const projectOptions = useMemo(() => {
    ...
    const linked = masters.projects.filter((project) => String(project.customerId || "") === selectedParty);
  ...
  const billedPoIds = useMemo(() => new Set(tx.supplierBills.flatMap((bill: any) => [String(bill.poId || ""), String(bill.sourceDocumentId || "")]).filter(Boolean)), [tx.supplierBills]);
  const approvedPurchaseOrders = useMemo(() => tx.purchaseOrders.filter((row: any) => {
    const rowStatus = String(row.status || "").toUpperCase();
    const number = String(row.poNumber || "").toUpperCase();
    return rowStatus === "APPROVED" && !number.startsWith("SUPQ-") && !billedPoIds.has(String(row.poId));
  ...
  async function call(action: string, payload: unknown) {
    ...
    if (itemsLoaded && body.result?.itemLinking?.created) {
  ...
  async function approve(recordType: RecordType, recordId: string) {
    try {
      setStatus("Approving…");
      const response = await fetch("/api/erp/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ target: "approvals", body: { payload: { recordType, recordId, decision: "APPROVE", note: "Transaction list" } } }) });
      const body = await response.json();
  ...
  async function createSupplierInvoiceFromPo(po: any) {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "Supplier Invoice conversion failed");
    } finally {
  ...
  function existingCount() {
    ...
    if (tab === "salesPayment") return tx.payments.filter((row) => row.partyType === "Customer").length;
    ...
    if (tab === "purchasePayment") return tx.payments.filter((row) => row.partyType === "Supplier").length;
  ...
  return <>
    ...
    {sectionMode === "menu" && <section className={`panel ${styles.chooser}`}>
      ...
      <div className={styles.chooserGrid}>
        <button type="button" className={styles.ctaCard} onClick={() => openMode("create")}><span className={styles.ctaTitle}>{section.createLabel}</span><span className={styles.ctaCopy}>Open the new-document workflow for this section.</span></button>
        <button type="button" className={`secondary ${styles.ctaCard}`} onClick={() => openMode("list")}><span className={styles.ctaTitle}>{section.listLabel}</span><span className={styles.ctaCopy}>Show only saved documents. Open any row to view, print or continue its workflow.</span></button>
      </div>
    </section>}
    ...
    {sectionMode === "create" && commercial && <form className="panel" onSubmit={submitCommercial}>
      <div className="form-title-row"><div><h3>{section.createLabel}</h3><p className="small">{supplierQuotation ? "Existing Item Master records can be selected, or a temporary supplier item can be typed without creating an Item Master record." : "Every saved row is linked to Item Master. Existing items are reused; a new Item Master record is automatically created only when the entered item does not already exist."}</p></div><span className="auto-badge">Document No: {nextDocumentNo || "AUTO"}</span></div>
      <div className="form-grid">
        <label>{salesSide ? "Customer" : "Supplier"}<input list={partyListId} value={partyInput} onChange={(event) => handlePartyInput(event.target.value)} placeholder={`Type ${salesSide ? "customer" : "supplier"} name or ID`} autoComplete="off" required /><datalist id={partyListId}>{partyOptions.map((row) => <option key={partyId(row)} value={partyDisplay(row)} />)}</datalist></label>
        ...
        <label>GST %<input name="gstRate" type="number" min="0" max="100" step="0.01" value={gstRate} onChange={(event) => setGstRate(event.target.value)} /></label>
        {tab === "salesInvoice" && <label>Revenue Account<input name="accountId" placeholder="ACC-4100" /></label>}
      </div>

      <h4>Items</h4>
      <TransactionItemLines lines={lines} items={items} supplierQuotation={supplierQuotation} onChange={setLines} />
      ...
      <div className="button-row"><button type="submit">Save Draft</button></div>
    </form>}

    {sectionMode === "create" && tab === "salesPayment" && <SalesPaymentStaged />}

    {sectionMode === "create" && tab === "supplierInvoice" && <>
      <section className="panel"><div className="form-title-row"><div><h3>Create New Supplier Invoice</h3><p className="small">Supplier Invoices are created from approved Purchase Orders so source-document, Item Master and three-way-match controls remain linked.</p></div><span className="auto-badge">{existingLoading ? "Loading…" : `${approvedPurchaseOrders.length} PO Ready`}</span></div></section>
