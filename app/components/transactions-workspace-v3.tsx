...
const poInvoiceEligible = (value: unknown) => ["APPROVED", "PART_RECEIVED", "RECEIVED", "PART_BILLED", "CONVERTED", "BILL_CREATED"].includes(String(value || "").toUpperCase());
...
export default function TransactionsWorkspaceV3() {
  ...
  function changeTab(value: Tab) { resetSectionState(); setTab(value); setSectionMode("menu"); syncUrl(module, value, "menu"); }
  function openMode(mode: Exclude<SectionMode, "menu">) { resetSectionState(); setSectionMode(mode); syncUrl(module, tab, mode); }
  function backToSection() { resetSectionState(); setSectionMode("menu"); syncUrl(module, tab, "menu"); }
  ...
  async function call(action: string, payload: unknown) {
    setStatus("Saving…"); const response = await fetch("/api/erp/transactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, payload }) }); const body = await response.json();
  ...
  function workflowActions(recordType: RecordType, id: string, rowStatus: string) {
    ...
    return <>{normalized === "DRAFT" && <button type="button" onClick={() => void approve(recordType, id)}>Approve</button>}
      {recordType === "supplierBill" && paymentEligible(normalized) && <Link prefetch={false} className="button-link" href={`/transactions?module=purchase&tab=purchasePayment&mode=create&sourceBill=${encodeURIComponent(id)}`}>Create Payment Entry</Link>}
    </>;
  }
  ...
  async function createSupplierInvoiceFromPo(po: any) {
    const poId = String(po.poId || ""); setConversionBusy(poId); setStatus("Creating Supplier Invoice from currently billable quantity…");
    try {
      ...
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || "Supplier Invoice conversion failed"); await loadTransactions(true); router.push(documentHref("supplierBill", body.createdId, "create"));
  ...
  function existingRows() {
    if (tab === "salesQuote") return tx.quotes.map((row) => <tr key={row.quoteId}><td><Link prefetch={false} href={documentHref("quote", row.quoteId, "list")}><strong>{row.quoteNumber}</strong></Link></td><td>{row.customerId}<br />{row.projectId}</td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("quote", row.quoteId)}{workflowActions("quote", row.quoteId, row.status)}</div></td></tr>);
    if (tab === "salesInvoice") return tx.invoices.map((row) => <tr key={row.invoiceId}><td><Link prefetch={false} href={documentHref("invoice", row.invoiceId, "list")}><strong>{row.invoiceNumber}</strong></Link></td><td>{row.customerId}<br />{row.projectId}</td><td>{money(row.totalAmount)}<br/><span className="small">Outstanding {money(row.outstandingAmount)}</span></td><td>{row.status}</td><td><div className="row-actions">{view("invoice", row.invoiceId)}{workflowActions("invoice", row.invoiceId, row.status)}</div></td></tr>);
    if (tab === "supplierQuote") return tx.supplierQuotes.map((row) => <tr key={row.poId}><td><Link prefetch={false} href={documentHref("purchaseOrder", row.poId, "list")}><strong>{row.poNumber}</strong></Link></td><td>{row.supplierId}<br />{row.projectId}</td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("purchaseOrder", row.poId)}{workflowActions("purchaseOrder", row.poId, row.status)}</div></td></tr>);
    if (tab === "purchaseOrder") return tx.purchaseOrders.map((row) => <tr key={row.poId}><td><Link prefetch={false} href={documentHref("purchaseOrder", row.poId, "list")}><strong>{row.poNumber}</strong></Link></td><td>{row.supplierId}<br />{row.projectId}</td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("purchaseOrder", row.poId)}{workflowActions("purchaseOrder", row.poId, row.status)}</div></td></tr>);
    if (tab === "supplierInvoice") return tx.supplierBills.map((row) => <tr key={row.billId}><td><Link prefetch={false} href={documentHref("supplierBill", row.billId, "list")}><strong>{row.billNumber || row.billId}</strong></Link></td><td>{row.supplierId}<br />{row.projectId}</td><td>{money(row.totalAmount)}<br/><span className="small">Outstanding {money(row.outstandingAmount ?? row.totalAmount)}</span></td><td>{row.status}</td><td><div className="row-actions">{view("supplierBill", row.billId)}{workflowActions("supplierBill", row.billId, row.status)}</div></td></tr>);
    if (tab === "salesPayment") return tx.payments.filter((row) => row.partyType === "Customer").map((row) => <tr key={row.paymentId}><td><Link prefetch={false} href={documentHref("payment", row.paymentId, "list")}><strong>{row.paymentNumber}</strong></Link></td><td>Customer: {row.partyId}<br />{row.projectId}</td><td>{money(row.amount)}</td><td>{row.status}{row.journalId?<><br/><span className="small">Finalized</span></>:null}</td><td>{view("payment", row.paymentId)}</td></tr>);
    if (tab === "purchasePayment") return tx.payments.filter((row) => row.partyType === "Supplier").map((row) => <tr key={row.paymentId}><td><Link prefetch={false} href={documentHref("payment", row.paymentId, "list")}><strong>{row.paymentNumber}</strong></Link></td><td>Supplier: {row.partyId}<br />{row.projectId}</td><td>{money(row.amount)}</td><td>{row.status}{row.journalId?<><br/><span className="small">Finalized</span></>:null}</td><td>{view("payment", row.paymentId)}</td></tr>);
    return tx.expenses.map((row) => <tr key={row.expenseId}><td><Link prefetch={false} href={documentHref("expense", row.expenseId, "list")}><strong>{row.expenseNumber}</strong></Link></td><td>{row.supplierId}<br />{row.projectId}</td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("expense", row.expenseId)}{workflowActions("expense", row.expenseId, row.status)}</div></td></tr>);
  }

  return <>
    <div className="page-heading"><div><h2>{title}</h2><p className="small">Create and existing-document workspaces stay separate. Accounting-posted documents continue through payment and stock workflows.</p></div></div>
    ...
    {sectionMode === "menu" && <section className={`panel ${styles.chooser}`}><div className={styles.chooserHeader}><h3>{section.title}</h3><p className="small">Select one workspace.</p></div><div className={styles.chooserGrid}><button type="button" className={styles.ctaCard} onClick={() => openMode("create")}><span className={styles.ctaTitle}>{section.createLabel}</span><span className={styles.ctaCopy}>Open the new-document workflow.</span></button><button type="button" className={`secondary ${styles.ctaCard}`} onClick={() => openMode("list")}><span className={styles.ctaTitle}>{section.listLabel}</span><span className={styles.ctaCopy}>Show only saved documents.</span></button></div></section>}
    {sectionMode !== "menu" && <section className={`panel ${styles.contextBar}`}><button type="button" className="secondary" onClick={backToSection}>← Back to {section.title}</button><span className={styles.contextTitle}>{sectionMode === "create" ? section.createLabel : section.listLabel}</span></section>}

    {sectionMode === "create" && commercial && <form className="panel" onSubmit={submitCommercial}>
      <div className="form-title-row"><div><h3>{section.createLabel}</h3><p className="small">{supplierQuotation ? "Select an existing Item or type a temporary supplier item. TEMP lines do not create Item Master records until PO conversion." : "All rows are Item Master linked. Stock Sales Invoices use controlled Update Stock and moving-average COGS on approval."}</p></div><span className="auto-badge">Document No: {nextDocumentNo || "AUTO"}</span></div>
      <div className="form-grid"><label>{salesSide ? "Customer" : "Supplier"}<input list={partyListId} value={partyInput} onChange={(event) => handlePartyInput(event.target.value)} placeholder={`Type ${salesSide ? "customer" : "supplier"} name or ID`} autoComplete="off" required/><datalist id={partyListId}>{partyOptions.map((row) => <option key={partyId(row)} value={partyDisplay(row)}/>)}</datalist></label><label>Project<select name="projectId" defaultValue=""><option value="">No project</option>{projectOptions.map((row) => <option key={row.projectId} value={row.projectId}>{row.projectName} ({row.projectId})</option>)}</select></label><label>{numberLabel}<input value={nextDocumentNo || "AUTO"} readOnly/></label><div></div><label>Date<input name="documentDate" type="date" required defaultValue={localDate()}/></label>{tab === "salesInvoice" && <label>Due Date<input name="dueDate" type="date" defaultValue={localDate(30)}/></label>}{(tab === "salesQuote" || tab === "supplierQuote") && <label>Valid Till<input name="expiryDate" type="date" defaultValue={localDate(7)}/></label>}<label>GST %<input name="gstRate" type="number" min="0" max="100" step="0.01" value={gstRate} onChange={(event) => setGstRate(event.target.value)}/></label>{tab === "salesInvoice" && <label>Revenue Account Override<input name="accountId" placeholder="Normally from Item Master"/><span className="small">Item Master account has priority.</span></label>}</div>
      <h4>Items</h4><TransactionItemLines lines={lines} items={items} supplierQuotation={supplierQuotation} onChange={setLines}/>
      <div style={{display:"flex",justifyContent:"flex-end",marginTop:18}}><div style={{width:"min(420px,100%)",border:"1px solid #e5ebf2",borderRadius:10,overflow:"hidden",background:"#fff"}}><div style={{display:"flex",justifyContent:"space-between",padding:"12px 14px"}}><span>Sub Total</span><strong>{money(subtotal)}</strong></div><div style={{display:"flex",justifyContent:"space-between",padding:"12px 14px"}}><span>GST {Number(gstRate||0).toFixed(2)}%</span><strong>{money(gstAmount)}</strong></div><div style={{display:"flex",justifyContent:"space-between",padding:14,background:"#f8fafc",fontSize:18}}><strong>Net Total</strong><strong>{money(netTotal)}</strong></div></div></div><div className="button-row"><button type="submit">Save Draft</button></div>
    </form>}

    {sectionMode === "create" && tab === "salesPayment" && <SalesPaymentStaged/>}
    {sectionMode === "create" && tab === "supplierInvoice" && <><section className="panel"><div className="form-title-row"><div><h3>Create New Supplier Invoice</h3><p className="small">Partial billing is allowed. Stock lines become billable only after Purchase Receipt; service/non-stock lines use remaining ordered quantity. Reopen the PO later to bill subsequent receipts.</p></div><span className="auto-badge">{existingLoading ? "Loading…" : `${supplierInvoiceReadyPos.length} PO Available`}</span></div></section><section className="panel table-wrap"><table className="data-table"><thead><tr><th>Purchase Order</th><th>Supplier</th><th>Project</th><th>Total</th><th>Lifecycle</th><th>Action</th></tr></thead><tbody>{!existingLoading && supplierInvoiceReadyPos.length === 0 && <tr><td colSpan={6}>No approved/part-received Purchase Orders are available for billing.</td></tr>}{supplierInvoiceReadyPos.map((po) => <tr key={po.poId}><td><Link prefetch={false} href={documentHref("purchaseOrder", po.poId, "create")}><strong>{po.poNumber || po.poId}</strong></Link></td><td>{po.supplierId || "—"}</td><td>{po.projectId || "—"}</td><td>{money(po.totalAmount)}</td><td>{po.status}</td><td><button type="button" disabled={Boolean(conversionBusy)} onClick={() => void createSupplierInvoiceFromPo(po)}>{conversionBusy === po.poId ? "Creating…" : "Create Supplier Invoice"}</button></td></tr>)}</tbody></table></section></>}
    {sectionMode === "create" && tab === "purchasePayment" && <PurchasePaymentStaged/>}
    {sectionMode === "create" && tab === "expense" && <form className="panel form-grid" onSubmit={submitExpense}><h3 className="form-title">Create New Expense</h3><label>Date<input name="expenseDate" type="date" required defaultValue={localDate()}/></label><label>Supplier<input list="expense-supplier-suggestions" value={expenseSupplierInput} onChange={(event) => handleExpenseSupplierInput(event.target.value)}/><datalist id="expense-supplier-suggestions">{masters.suppliers.map((row) => <option key={partyId(row)} value={partyDisplay(row)}/>)}</datalist></label><label>Project<select name="projectId" defaultValue=""><option value="">No project</option>{masters.projects.map((row) => <option key={row.projectId} value={row.projectId}>{row.projectName}</option>)}</select></label><label>Expense Account<input name="expenseAccountId" defaultValue="ACC-6600" required/></label><label>Net Amount<input name="netAmount" type="number" min="0" step="0.01" required/></label><label>GST Amount<input name="gstAmount" type="number" min="0" step="0.01" defaultValue="0"/></label><label>Payment Method<select name="paymentMethod"><option>Cash</option><option>Bank Transfer</option><option>Card</option></select></label><label>Cash / Bank Account<input name="cashBankAccountId" defaultValue="ACC-1110" required/></label><label className="form-wide">Description<input name="description" required/></label><div className="form-wide"><button type="submit">Save Draft</button></div></form>}
