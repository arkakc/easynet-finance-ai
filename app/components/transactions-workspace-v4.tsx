...
const supplierQuoteConvertEligible = (value: unknown) => ["APPROVED", "CONVERTED"].includes(String(value || "").toUpperCase());
...
function ascending<T>(rows: T[]) { return [...rows].sort((a: any, b: any) => createdValue(a) - createdValue(b)); }
...
export default function TransactionsWorkspaceV4() {
  ...
  function workflowActions(recordType: RecordType, id: string, rowStatus: string) {
    ...
    return <>{normalized === "DRAFT" && <button type="button" disabled={globallyBusy} onClick={() => void approve(recordType, id)}>{approvalBusy === id ? "Approving…" : "Approve"}</button>}
  ...
  async function convertSupplierQuoteToPo(row: any) {
    const id = String(row.poId || ""); if (!id || globallyBusy) return;
    setConversionBusy(`quote:${id}`); setStatus("Saving Purchase Order from Supplier Quotation…");
    try {
      const response = await fetch("/api/erp/purchase-conversions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ supplierQuoteId: id }) });
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || "Supplier Quotation conversion failed");
      await loadTransactions(true); setReceiptStatesLoaded(false); router.push(documentHref("purchaseOrder", body.createdId, "list"));
    } catch (error) { setStatus(error instanceof Error ? error.message : "Supplier Quotation conversion failed"); }
  ...
  function existingRows() {
    ...
    if (tab === "supplierQuote") return ascending(tx.supplierQuotes).map((row) => <tr key={row.poId}><td><Link prefetch={false} href={documentHref("purchaseOrder", row.poId, "list")}><strong>{row.poNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{row.supplierId}<br />{row.projectId}</td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("purchaseOrder", row.poId)}{workflowActions("purchaseOrder", row.poId, row.status)}{supplierQuoteConvertEligible(row.status) && <button type="button" disabled={globallyBusy} onClick={() => void convertSupplierQuoteToPo(row)}>{conversionBusy === `quote:${row.poId}` ? "Saving…" : "Convert to Purchase Order"}</button>}</div></td></tr>);
