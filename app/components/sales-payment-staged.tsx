...
export default function SalesPaymentStaged() {
  ...
  const approved = useMemo(() => invoices.filter((row) => paymentEligibleStatus(row.status) && Number(row.outstandingAmount ?? row.totalAmount ?? 0) > 0), [invoices]);
  ...
  async function openDirectMode() {
    setMode("direct"); setStatus(""); setCustomerInput(""); setSelectedCustomer("");
    await Promise.all([loadMasters(), loadNextNo()]);
  }
  function backToPaymentChoices() {
    setMode("menu"); setStatus(""); setSearch(""); setSearched(null); setSelectedInvoice(null); setCustomerInput(""); setSelectedCustomer("");
  }
  function selectInvoice(row: any) {
    setSelectedInvoice(row); setSearched(row); setStatus("");
  ...
  async function saveInvoicePayment(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setStatus("Saving Payment Entry…");
    try {
      ...
      const result = await createPayment({
        paymentNumber: "", paymentType: "RECEIVE", partyType: "Customer", partyId: selectedInvoice.customerId,
        projectId: selectedInvoice.projectId || "", paymentDate: form.get("paymentDate"), amount,
        paymentMethod: form.get("paymentMethod"), cashBankAccountId: form.get("cashBankAccountId"), reference: form.get("reference") || "",
        againstDocumentType: "Sales Invoice", againstDocumentId: selectedInvoice.invoiceId,
      });
      router.push(`/transactions/payment/${result.recordId}?${returnQuery}`); router.refresh();
    } catch (error) { setStatus(error instanceof Error ? error.message : "Sales Payment draft save failed"); }
    finally { setBusy(false); }
  }

  async function saveDirectPayment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setBusy(true); setStatus("Saving Customer Advance…");
    try {
      const form = new FormData(event.currentTarget);
      ...
      if (!(amount > 0)) throw new Error("Payment amount must be greater than zero");
      const result = await createPayment({
        paymentNumber: "", paymentType: "RECEIVE", partyType: "Customer", partyId: customerId(match), projectId: form.get("projectId") || "",
        paymentDate: form.get("paymentDate"), amount, paymentMethod: form.get("paymentMethod"), cashBankAccountId: form.get("cashBankAccountId"),
        reference: form.get("reference") || "", againstDocumentType: "", againstDocumentId: "",
      });
      router.push(`/transactions/payment/${result.recordId}?${returnQuery}`); router.refresh();
    } catch (error) { setStatus(error instanceof Error ? error.message : "Customer Advance save failed"); }
    finally { setBusy(false); }
  }
  ...
  return <>
    ...
    {mode === "menu" && <section className="panel">
      <div className="form-title-row"><div><h3>Create New Sales Payment Entry / Receipt</h3><p className="small">Choose an allocated receipt against a posted Sales Invoice, or record money received before an invoice as a Customer Advance.</p></div></div>
      <div className="button-row" style={{ marginTop: 18, alignItems: "stretch" }}>
        <button type="button" style={{ minHeight: 86, flex: "1 1 320px", textAlign: "left" }} onClick={() => void openInvoiceMode()}><strong style={{ display: "block" }}>Approved Sales Invoices Pending Payment</strong><span style={{ display: "block", marginTop: 7, fontWeight: 400 }}>Posted/part-paid invoices with an outstanding balance.</span></button>
        <button type="button" className="secondary" style={{ minHeight: 86, flex: "1 1 320px", textAlign: "left" }} onClick={() => void openDirectMode()}><strong style={{ display: "block" }}>Direct Payment Entry Without Sales Invoice</strong><span style={{ display: "block", marginTop: 7, fontWeight: 400 }}>Records a Customer Advance / contract liability until allocated.</span></button>
      </div>
    </section>}

    {mode !== "menu" && <section className="panel"><div className="button-row" style={{ justifyContent: "space-between" }}><button type="button" className="secondary" onClick={backToPaymentChoices}>← Back to Create Sales Payment Entry / Receipt</button><span className="auto-badge">Next Payment No: {nextNo || "AUTO"}</span></div></section>}
    ...
    {mode === "invoice" && <>
      <section className="panel table-wrap">
        ...
        <table className="data-table"><thead><tr><th>Sales Invoice</th><th>Customer</th><th>Project</th><th>Invoice Total</th><th>Outstanding</th><th>Action</th></tr></thead><tbody>
          {!loading && approved.length === 0 && <tr><td colSpan={6}>No posted Sales Invoices with outstanding balance.</td></tr>}
      ...
      <section className="panel"><h3>Search Posted Sales Invoice</h3><div className="form-grid" style={{ marginTop: 16 }}><label>Sales Invoice No<input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void searchInvoice(); } }} placeholder="e.g. SI-2026-00001" autoComplete="off" /></label><div style={{ display: "flex", alignItems: "end" }}><button type="button" style={{ width: "100%", height: 52 }} onClick={() => void searchInvoice()} disabled={loading}>{loading ? "Searching…" : "Search"}</button></div></div>
    ...
    {mode === "direct" && <form className="panel form-grid" onSubmit={saveDirectPayment}>
      ...
      <label>Project<select name="projectId" defaultValue=""><option value="">No project</option>{projectOptions.map((row) => <option key={row.projectId} value={row.projectId}>{row.projectName} ({row.projectId})</option>)}</select></label><label>Auto Payment No<input value={nextNo || "AUTO"} readOnly /></label><label>Payment Date<input name="paymentDate" type="date" defaultValue={localDate()} required /></label><label>Amount<input name="amount" type="number" min="0.01" step="0.01" required /></label><label>Payment Method<select name="paymentMethod" defaultValue="" required><option value="">Select payment method</option><option>Cash</option><option>Bank Transfer</option><option>Card</option><option>Cheque</option></select></label><label>Cash / Bank Account<input name="cashBankAccountId" placeholder="e.g. ACC-1120" required /></label><label className="form-wide">Reference<input name="reference" placeholder="Bank / receipt reference" /></label>
      <div className="form-wide"><p className="small">Accounting on Final Save: Dr Cash/Bank, Cr Customer Advances / Unearned Revenue (ACC-2150). It does not reduce Accounts Receivable until allocated to a posted Sales Invoice.</p></div><div className="form-wide button-row"><button type="submit" disabled={busy || loading}>{busy ? "Saving Draft…" : "Save Customer Advance Draft"}</button></div>
    </form>}
  </>;
}
