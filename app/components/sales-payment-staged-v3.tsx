...
export default function SalesPaymentStagedV3(){
  ...
  const[masters,setMasters]=useState<Master>({customers:[],projects:[]}),[accounts,setAccounts]=useState<CashBankAccount[]>([]),[customerInput,setCustomerInput]=useState(""),[selectedCustomer,setSelectedCustomer]=useState(""),[cashBankAccountId,setCashBankAccountId]=useState("");
  ...
  function backToPaymentChoices(){setMode("menu");setStatus("");setSearch("");setSearched(null);setSelectedInvoice(null);setCustomerInput("");setSelectedCustomer("");setCashBankAccountId("");}
  ...
  const cashBankOptions=<><option value="">Select Cash / Bank account</option>{accounts.map(account=><option key={account.accountId} value={account.accountId}>{account.accountCode || account.accountId} — {account.accountName || account.accountId} · Balance {money(account.balance)}</option>)}</>;
