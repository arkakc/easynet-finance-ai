...
type AccountOption = { accountId: string; accountCode: string; accountName: string; accountType: string; parentAccount: string; balance: number };
...
function initialLines(type: Props["type"], rows: any[], items: TransactionItemMaster[]): TransactionDraftLine[] {
  ...
  return rows.map((row) => {
    ...
    return { lineId: lineIdFor(type, row), itemId, itemInput: item ? itemDisplay(item) : name, itemName: name, itemType: String(item?.itemType || "STOCK"), description: String(row.description || name), qty: String(row.qty ?? 1), uom: String(row.uom || item?.uom || "Each"), rate: String(row.rate ?? 0) };
  });
}
function projectDisplay(row: any) { const id=String(row.projectId||""); const name=String(row.projectName||id); return name!==id?`${name} (${id})`:id; }
...
export default function DraftDocumentEditor(props: Props) {
  ...
  const initialRate = Number(props.record.netAmount || 0) > 0 ? Number(props.record.gstAmount || 0) / Number(props.record.netAmount || 0) : 0.1;
  const [gstRate, setGstRate] = useState(String(Number((initialRate * 100).toFixed(4))));

  useEffect(() => {
    if (!["payment", "expense"].includes(props.type)) return;
    let active=true;
    void (async()=>{
      try {
        ...
        if(!active)return;
        setAccounts(body.accounts||[]); setCashBankAccounts(body.cashBankAccounts||[]);
      } catch(error){ if(active)setStatus(error instanceof Error?error.message:"Account options load failed"); }
    })();
    return()=>{active=false;};
  },[props.type]);

  const parentIds=useMemo(()=>new Set(accounts.map((row)=>String(row.parentAccount||"")).filter(Boolean)),[accounts]);
  const expenseAccounts=useMemo(()=>accounts.filter((row)=>String(row.accountType||"").toLowerCase()==="expense"&&!parentIds.has(row.accountId)),[accounts,parentIds]);
  ...
  async function saveCommercial(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (saving) return; setSaving(true); setStatus("Saving...");
    try {
      const form = new FormData(event.currentTarget);
      const response = await fetch("/api/erp/draft-document", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: props.type, id: props.id, partyId, projectId, documentDate: form.get("documentDate"), dueDate: form.get("dueDate") || "", expiryDate: form.get("expiryDate") || "", gstRate: (Number(gstRate) || 0) / 100, lines: lines.map((line) => ({ lineId: line.lineId || "", itemId: line.itemId, itemName: line.itemName, itemType: line.itemType, description: line.description || line.itemName, qty: Number(line.qty), uom: line.uom, rate: Number(line.rate) })) }) });
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || "Save failed");
      setStatus("Draft saved successfully."); router.push(backHref); router.refresh();
    } catch (error) { setStatus(error instanceof Error ? error.message : "Save failed"); } finally { setSaving(false); }
  }
  async function savePayment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (saving) return; setSaving(true); setStatus("Saving...");
    try {
      const form = new FormData(event.currentTarget);
      const response = await fetch("/api/erp/draft-document", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "payment", id: props.id, partyId, projectId, paymentDate: form.get("paymentDate"), amount: Number(form.get("amount") || 0), paymentMethod: form.get("paymentMethod"), cashBankAccountId: form.get("cashBankAccountId"), reference: form.get("reference") || "" }) });
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || "Save failed");
      setStatus("Draft saved successfully."); router.push(backHref); router.refresh();
    } catch (error) { setStatus(error instanceof Error ? error.message : "Save failed"); } finally { setSaving(false); }
  }
  async function saveExpense(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const form = new FormData(event.currentTarget);
      const response = await fetch("/api/erp/draft-document", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "expense", id: props.id, supplierId: form.get("supplierId") || "", projectId, expenseDate: form.get("expenseDate"), expenseAccountId: form.get("expenseAccountId"), description: form.get("description"), netAmount: Number(form.get("netAmount") || 0), gstAmount: Number(form.get("gstAmount") || 0), paymentMethod: form.get("paymentMethod"), cashBankAccountId: form.get("cashBankAccountId") }) });
      const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || "Save failed");
      setStatus("Draft saved successfully."); router.push(backHref); router.refresh();
    } catch (error) { setStatus(error instanceof Error ? error.message : "Save failed"); } finally { setSaving(false); }
  }

  const cashBankSelect=(name:string,defaultValue:string)=><select name={name} defaultValue={defaultValue} required disabled={saving}><option value="">Select Cash / Bank account</option>{cashBankAccounts.map((row)=><option key={row.accountId} value={row.accountId}>{row.accountName} ({row.accountId}) · Balance {money(row.balance)}</option>)}</select>;

  return <div className="document-page">
    <div className="document-toolbar no-print"><Link prefetch={false} href={backHref}>← Cancel Edit</Link><strong>Edit Draft · {props.number}</strong></div>
    ...
    {commercial && <form className="panel" onSubmit={saveCommercial}>
      ...
      <div className="form-grid">
        ...
        <label>Project<select value={projectId} onChange={(event) => setProjectId(event.target.value)} disabled={saving}><option value="">No project</option>{props.projects.map((row: any) => <option key={row.projectId} value={row.projectId}>{projectDisplay(row)}</option>)}</select></label>
        <label>Document ID / Number<input value={props.number} readOnly disabled /></label><div></div>
      ...
      <h4>Items</h4><TransactionItemLines lines={lines} items={props.items} temporaryQuotation={temporaryQuotation} masterOnly={!temporaryQuotation} disabled={saving} onChange={setLines} />
    ...
    {props.type === "payment" && <form className="panel form-grid" onSubmit={savePayment}>
      ...
      <label>Payment Method<select name="paymentMethod" defaultValue={String(props.record.paymentMethod||"")} required disabled={saving}><option value="">Select method</option><option>Cash</option><option>Bank Transfer</option><option>Card</option><option>Cheque</option></select></label>
      <label>Cash / Bank Account{cashBankSelect("cashBankAccountId",String(props.record.cashBankAccountId||""))}</label>
    ...
    {props.type === "expense" && <form className="panel form-grid" onSubmit={saveExpense}>
      ...
      <label>Project<select value={projectId} onChange={(event) => setProjectId(event.target.value)} disabled={saving}><option value="">No project</option>{props.projects.map((row: any) => <option key={row.projectId} value={row.projectId}>{projectDisplay(row)}</option>)}</select></label>
      <label>Expense Account<select name="expenseAccountId" defaultValue={String(props.record.expenseAccountId || "ACC-6600")} required disabled={saving}><option value="">Select Expense account</option>{expenseAccounts.map((row)=><option key={row.accountId} value={row.accountId}>{row.accountName} ({row.accountId}) · {row.accountCode}</option>)}</select></label>
