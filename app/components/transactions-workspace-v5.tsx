...
export default function TransactionsWorkspaceV5(){
  ...
  const[quickModal,setQuickModal]=useState<MasterType|null>(null);const[selectedProjectId,setSelectedProjectId]=useState<string>("");const[invoiceAccountOverride,setInvoiceAccountOverride]=useState("");const[expenseAccountId,setExpenseAccountId]=useState("");const[cashBankAccountId,setCashBankAccountId]=useState("");
  ...
  const loadAccountOptions=useCallback(async()=>{try{const response=await fetch("/api/erp/reference-options",{cache:"no-store"});const body=await response.json();if(!response.ok||!body.ok)throw new Error(body.error||"Account options load failed");setAccountOptions(Array.isArray(body.accounts)?body.accounts:[]);}catch(error){setStatus(error instanceof Error?error.message:"Account options load failed");}},[]);
  ...
  function resetSectionState(){setStatus("");setSelectedParty("");setPartyInput("");setSelectedProjectId("");setExpenseSupplier("");setExpenseSupplierInput("");setInvoiceAccountOverride("");setExpenseAccountId("");setCashBankAccountId("");setLines([emptyTransactionLine()]);}
