...
const numberMeta:Partial<Record<Tab,{label:string;action:string;partyType?:string}>>={
  salesQuote:{label:"Auto Quotation No",action:"createQuote"}, salesInvoice:{label:"Auto Sales Invoice No",action:"createInvoice"}, supplierQuote:{label:"Auto Supplier Quotation No",action:"createSupplierQuote"}, purchaseOrder:{label:"Auto Purchase Order No",action:"createPurchaseOrder"},
};
...
function createdValue(row:any){const value=row.createdAt||row.creation||row.created||row.updatedAt||row.quoteDate||row.invoiceDate||row.poDate||row.billDate||row.paymentDate||row.expenseDate||"";const t=new Date(value).getTime();return Number.isFinite(t)?t:0;}
function createdLabel(row:any){const value=row.createdAt||row.creation||row.created||row.updatedAt||"";if(!value)return"—";const d=new Date(value);if(!Number.isFinite(d.getTime()))return String(value);return new Intl.DateTimeFormat("en-PG",{timeZone:"Pacific/Port_Moresby",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(d);}
...
export default function TransactionsWorkspaceV5(){
  ...
  const[status,setStatus]=useState("");const[selectedParty,setSelectedParty]=useState("");const[partyInput,setPartyInput]=useState("");const[expenseSupplier,setExpenseSupplier]=useState("");const[expenseSupplierInput,setExpenseSupplierInput]=useState("");const[lines,setLines]=useState<TransactionDraftLine[]>([emptyTransactionLine()]);const[gstRate,setGstRate]=useState("10");const[nextDocumentNo,setNextDocumentNo]=useState("AUTO");const[saving,setSaving]=useState(false);const[approvalBusy,setApprovalBusy]=useState("");const[conversionBusy,setConversionBusy]=useState("");
  const[quickModal,setQuickModal]=useState<MasterType|null>(null);const[selectedProjectId,setSelectedProjectId]=useState<string>("");
  const[deleteBusy,setDeleteBusy]=useState<string>("");

  function syncUrl(nextModule:Module,nextTab:Tab,nextMode:SectionMode){if(typeof window==="undefined")return;const params=new URLSearchParams(window.location.search);params.set("module",nextModule);params.set("tab",nextTab);params.set("mode",nextMode);window.history.replaceState(window.history.state,"",`${window.location.pathname}?${params.toString()}`);}
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);const requestedModule=params.get("module");const resolvedModule:Module=requestedModule==="purchase"||requestedModule==="expense"?requestedModule:"sales";const requestedTab=params.get("tab") as Tab|null;const resolvedTab=requestedTab&&MODULE_TABS[resolvedModule].includes(requestedTab)?requestedTab:defaultTab(resolvedModule);const requestedMode=params.get("mode");const resolvedMode:SectionMode=requestedMode==="create"?requestedMode:"list";setModule(resolvedModule);setTab(resolvedTab);setSectionMode(resolvedMode);syncUrl(resolvedModule,resolvedTab,resolvedMode);setInitialized(true);},[]);
  useEffect(()=>{if(!initialized)return;if(sectionMode==="list"){void loadTransactions(true);if(!mastersLoaded)void loadMasters();if(tab==="purchaseOrder"&&!receiptStatesLoaded)void loadReceiptStates();}if(sectionMode==="create"){if(["salesQuote","salesInvoice","supplierQuote","purchaseOrder","supplierInvoice","expense"].includes(tab)&&!mastersLoaded)void loadMasters();if(["salesQuote","salesInvoice","supplierQuote","purchaseOrder"].includes(tab)&&!itemsLoaded)void loadItems();if(tab==="supplierInvoice"&&!existingLoaded)void loadTransactions();if(numberMeta[tab])void loadNextDocumentNo(tab);}},[initialized,sectionMode,tab]);

  const salesSide=module==="sales";const commercial=["salesQuote","salesInvoice","supplierQuote","purchaseOrder"].includes(tab);const supplierQuotation=tab==="supplierQuote";const partyOptions=salesSide?masters.customers:masters.suppliers;
  ...
  function resetSectionState(){setStatus("");setSelectedParty("");setPartyInput("");setSelectedProjectId("");setExpenseSupplier("");setExpenseSupplierInput("");setLines([emptyTransactionLine()]);}
  ...
  function editAction(recordType:RecordType,id:string,rowStatus:string){const normalized=String(rowStatus||"DRAFT").toUpperCase();if(normalized!=="DRAFT")return<button type="button" disabled>Edit Locked</button>;const href=recordType==="invoice"?`/transactions/invoice/${encodeURIComponent(id)}/edit`:`/transactions/${recordType}/${encodeURIComponent(id)}/edit`;return<Link prefetch={false} className="button-link" href={href}>Edit</Link>;}
  function workflowActions(recordType:RecordType,id:string,rowStatus:string){const normalized=String(rowStatus||"DRAFT").toUpperCase();return<>{editAction(recordType,id,normalized)}{normalized==="DRAFT"&&<button type="button" disabled={globallyBusy} onClick={()=>void approve(recordType,id)}>{approvalBusy===id?"Approving…":"Approve"}</button>}{recordType==="supplierBill"&&paymentEligible(normalized)&&<a className="button-link" href={`/transactions?module=purchase&tab=purchasePayment&mode=create&sourceBill=${encodeURIComponent(id)}`}>Create Payment Entry</a>}</>;}

  async function submitCommercial(event:FormEvent<HTMLFormElement>){event.preventDefault();if(globallyBusy)return;const saveName=supplierQuotation?"Supplier Quotation":SECTION_META[tab].title;setSaving(true);setStatus(`Saving ${saveName}…`);try{const form=new FormData(event.currentTarget);const resolvedParty=validateParty(partyOptions,partyInput,selectedParty,salesSide?"Customer":"Supplier");const payload={documentNumber:"",partyId:resolvedParty,projectId:form.get("projectId")??"",documentDate:form.get("documentDate"),dueDate:form.get("dueDate")??"",expiryDate:form.get("expiryDate")??"",gstRate:Number(form.get("gstRate")||0)/100,accountId:form.get("accountId")??"",poId:"",lines:lines.map(line=>({itemId:line.itemId,itemCode:line.itemId,itemName:line.itemName,itemType:line.itemType,description:line.itemName||line.description,qty:Number(line.qty),uom:line.uom,rate:Number(line.rate)}))};const action=tab==="salesQuote"?"createQuote":tab==="salesInvoice"?"createInvoice":tab==="supplierQuote"?"createSupplierQuote":"createPurchaseOrder";const result=await call(action,payload);const detail=tab==="salesQuote"?"quote":tab==="salesInvoice"?"invoice":"purchaseOrder";router.push(documentHref(detail,result.recordId,"list"));}catch(error){setStatus(error instanceof Error?error.message:"Save failed");}finally{setSaving(false);}}
  ...
  function existingCount(){if(tab==="salesQuote")return tx.quotes.length;if(tab==="salesInvoice")return tx.invoices.length;if(tab==="salesPayment")return tx.payments.filter(r=>r.partyType==="Customer").length;if(tab==="supplierQuote")return tx.supplierQuotes.length;if(tab==="purchaseOrder")return tx.purchaseOrders.length;if(tab==="supplierInvoice")return tx.supplierBills.length;if(tab==="purchasePayment")return tx.payments.filter(r=>r.partyType==="Supplier").length;return tx.expenses.length;}
  function existingRows(){
    if(tab==="salesQuote")return newest(tx.quotes).map(row=><tr key={row.quoteId}><td><Link href={documentHref("quote",row.quoteId,"list")}><strong>{row.quoteNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{customerLabel(row.customerId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("quote",row.quoteId)}{workflowActions("quote",row.quoteId,row.status)}{deleteAction("quote",row.quoteId,row.quoteNumber)}</div></td></tr>);
    if(tab==="salesInvoice")return newest(tx.invoices).map(row=><tr key={row.invoiceId} className={Number(row.outstandingAmount||0)>0?styles.outstandingRow:""}><td><Link href={documentHref("invoice",row.invoiceId,"list")}><strong>{row.invoiceNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{customerLabel(row.customerId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}<br/><span className="small">Outstanding {money(row.outstandingAmount)}</span></td><td>{row.status}</td><td><div className="row-actions">{view("invoice",row.invoiceId)}{workflowActions("invoice",row.invoiceId,row.status)}{deleteAction("invoice",row.invoiceId,row.invoiceNumber)}</div></td></tr>);
    if(tab==="supplierQuote")return newest(tx.supplierQuotes).map(row=>{const convertedPo=tx.purchaseOrders.find(po=>String(po.sourceDocumentId||"")===String(row.poId||""));const approved=String(row.status||"").toUpperCase()==="APPROVED";return<tr key={row.poId}><td><Link href={documentHref("purchaseOrder",row.poId,"list")}><strong>{row.poNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{supplierLabel(row.supplierId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}</td><td>{convertedPo?"CONVERTED":row.status}{convertedPo?<><br/><span className="small">→ {convertedPo.poNumber||convertedPo.poId}</span></>:null}</td><td><div className="row-actions">{view("purchaseOrder",row.poId)}{editAction("purchaseOrder",row.poId,row.status)}{String(row.status||"").toUpperCase()==="DRAFT"&&<button type="button" disabled={globallyBusy} onClick={()=>void approve("purchaseOrder",row.poId)}>{approvalBusy===row.poId?"Approving…":"Approve"}</button>}{convertedPo?<Link className="button-link" href={documentHref("purchaseOrder",convertedPo.poId,"list")}>Open Purchase Order</Link>:approved?<Link className="button-link" href={documentHref("purchaseOrder",row.poId,"list")}>Prepare Items / Create PO</Link>:null}{deleteAction("purchaseOrder",row.poId,row.poNumber)}</div></td></tr>;});
    if(tab==="purchaseOrder")return newest(tx.purchaseOrders).map(row=>{const receipt=receiptStates[String(row.poId||"")];const rowClass=receipt?.partial?styles.partialReceiptRow:receipt?.fullyReceived?styles.receiptCompleteRow:"";const eligible=poInvoiceEligible(row.status);return<tr key={row.poId} className={rowClass}><td><Link href={documentHref("purchaseOrder",row.poId,"list")}><strong>{row.poNumber}</strong></Link>{receipt?.hasStock&&<><br/><span className="small">Received {receipt.received.toLocaleString()} / {receipt.ordered.toLocaleString()} · Remaining {receipt.remaining.toLocaleString()}</span></>}</td><td>{createdLabel(row)}</td><td>{supplierLabel(row.supplierId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}</td><td>{row.status}{receipt?.partial&&<><br/><span className="small"><strong>PARTIAL RECEIPT</strong></span></>}{receipt?.fullyReceived&&<><br/><span className="small">Goods fully received</span></>}</td><td><div className="row-actions">{view("purchaseOrder",row.poId)}{workflowActions("purchaseOrder",row.poId,row.status)}{eligible&&receipt?.hasStock&&(receipt.hasRemaining&&String(row.status||"").toUpperCase()!=="CLOSED_PARTIAL"?<Link className="button-link" href={`/stock?mode=movement&sourcePo=${encodeURIComponent(row.poId)}`}>Create Purchase Receipt / GRN</Link>:<button type="button" disabled>{String(row.status||"").toUpperCase()==="CLOSED_PARTIAL"?"PO Closed — No More Receipt":"Purchase Receipt Complete"}</button>)}{eligible&&<button type="button" disabled={globallyBusy} onClick={()=>void createSupplierInvoiceFromPo(row,"list")}>{conversionBusy===`bill:${row.poId}`?"Saving…":"Create Supplier Invoice"}</button>}{deleteAction("purchaseOrder",row.poId,row.poNumber)}</div></td></tr>;});
    if(tab==="supplierInvoice")return newest(tx.supplierBills).map(row=>{const outstanding=Number(row.outstandingAmount??row.totalAmount??0);const partialPaid=String(row.status||"").toUpperCase()==="PARTLY_PAID";return<tr key={row.billId} className={outstanding>0?(partialPaid?styles.partialPaidRow:styles.outstandingRow):""}><td><Link href={documentHref("supplierBill",row.billId,"list")}><strong>{row.billNumber||row.billId}</strong></Link></td><td>{createdLabel(row)}</td><td>{supplierLabel(row.supplierId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}<br/><span className="small"><strong>Outstanding {money(outstanding)}</strong></span></td><td>{row.status}</td><td><div className="row-actions">{view("supplierBill",row.billId)}{workflowActions("supplierBill",row.billId,row.status)}{deleteAction("supplierBill",row.billId,row.billNumber||row.billId)}</div></td></tr>;});
    if(tab==="salesPayment")return newest(tx.payments.filter(r=>r.partyType==="Customer")).map(row=><tr key={row.paymentId}><td><Link href={documentHref("payment",row.paymentId,"list")}><strong>{row.paymentNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{customerLabel(row.partyId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.amount)}</td><td>{row.status}{row.journalId&&<><br/><span className="small">Finalized</span></>}</td><td><div className="row-actions">{view("payment",row.paymentId)}{editAction("payment",row.paymentId,row.status)}{deleteAction("payment",row.paymentId,row.paymentNumber)}</div></td></tr>);
    if(tab==="purchasePayment")return newest(tx.payments.filter(r=>r.partyType==="Supplier")).map(row=><tr key={row.paymentId}><td><Link href={documentHref("payment",row.paymentId,"list")}><strong>{row.paymentNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{supplierLabel(row.partyId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.amount)}</td><td>{row.status}{row.journalId&&<><br/><span className="small">Finalized</span></>}</td><td><div className="row-actions">{view("payment",row.paymentId)}{editAction("payment",row.paymentId,row.status)}{row.journalId&&<Link className="button-link" href={`/journals/${encodeURIComponent(row.journalId)}`}>View Journal</Link>}{deleteAction("payment",row.paymentId,row.paymentNumber)}</div></td></tr>);
    return newest(tx.expenses).map(row=><tr key={row.expenseId}><td><Link href={documentHref("expense",row.expenseId,"list")}><strong>{row.expenseNumber}</strong></Link></td><td>{createdLabel(row)}</td><td>{supplierLabel(row.supplierId)}<br/><span className="small">{projectLabel(row.projectId)}</span></td><td>{money(row.totalAmount)}</td><td>{row.status}</td><td><div className="row-actions">{view("expense",row.expenseId)}{workflowActions("expense",row.expenseId,row.status)}{deleteAction("expense",row.expenseId,row.expenseNumber)}</div></td></tr>);
  }
  ...
  return <>
    <div className="page-head">
      <div>
        <h2>{title}</h2>
        <p className="small">Commercial trade transactions, multi-line pricing, and automated general ledger posting.</p>
      </div>
    ...
    {sectionMode==="create"&&commercial&&<form className="panel" onSubmit={submitCommercial}>
      ...
      <div className="form-grid">
        <label>{salesSide?"Customer":"Supplier"}<input list={partyListId} value={partyInput} onChange={event=>handlePartyInput(event.target.value)} placeholder={`Type ${salesSide?"customer":"supplier"} name or ID`} autoComplete="off" required disabled={saving}/><datalist id={partyListId}>{partyOptions.map(row=><option key={partyId(row)} value={partyDisplay(row)}/>)}</datalist><div className="field-action"><button type="button" onClick={()=>setQuickModal(salesSide?"customer":"supplier")}>{`+ Create New ${salesSide?"Customer":"Supplier"}`}</button></div></label>
