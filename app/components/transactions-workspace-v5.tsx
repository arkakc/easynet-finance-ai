  ...
  return <>
    ...
    {sectionMode==="create"&&commercial&&<form className="panel" onSubmit={submitCommercial} onInvalidCapture={invalidCommercialField}>
      ...
      <div className="form-grid">
        <label>{salesSide?"Customer":"Supplier"}<select value={selectedParty} onChange={event=>{const row=partyOptions.find(option=>partyId(option)===event.target.value);handlePartyInput(row?partyDisplay(row):"");}} required disabled={saving}><option value="" disabled>{`Select ${salesSide?"customer":"supplier"}`}</option>{partyOptions.map(row=><option key={partyId(row)} value={partyId(row)}>{partyDisplay(row)}</option>)}</select><div className="field-action"><button type="button" onClick={()=>setQuickModal(salesSide?"customer":"supplier")}>{`+ Create New ${salesSide?"Customer":"Supplier"}`}</button></div></label>
