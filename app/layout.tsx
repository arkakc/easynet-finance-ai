...
export default async function RootLayout({children}:{children:React.ReactNode}){const user=await getCurrentUser();return <html lang="en"><body>{!user?children:<div className="shell"><aside className="sidebar"><Link prefetch={false} className="brand" href="/dashboard"><strong>EASYNET FINANCE AI</strong><span>Finance ERP</span></Link><nav>{groups.map(group=>{const visible=group.links.filter(([, ,permission])=>hasPermission(user,permission));if(!visible.length)return null;return <div className="nav-section" key={group.label}><div className="nav-label">{group.label}</div>{visible.map(([label,href])=><Link prefetch={false} href={href} key={`${group.label}-${label}`}>{label}</Link>)}</div>})}</nav><div className="sidebar-user"><strong>{user.name}</strong><span>{user.roles.join(", ")}</span><LogoutButton/></div></aside><main className="main"><FlowReturnBridge/>{children}</main></div>}</body></html>}
