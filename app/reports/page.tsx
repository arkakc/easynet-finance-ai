...
type JournalLine = { accountId: string; debit: number | string; credit: number | string; projectId?: string };
type Invoice = { invoiceId: string; invoiceNumber: string; customerId: string; dueDate: string; totalAmount: number | string; outstandingAmount: number | string; status: string; projectId?: string };
type Bill = { billId: string; billNumber: string; supplierId: string; dueDate: string; totalAmount: number | string; outstandingAmount: number | string; status: string; projectId?: string };
...
export default async function ReportsPage() {
  ...
  try {
    const [a, j, i, b] = await Promise.all([
  ...
  const arRows = invoices.filter((row) => n(row.outstandingAmount) > 0 && row.status !== "DRAFT").map((row) => ({ ...row, overdueDays: daysPastDue(row.dueDate), aging: bucket(daysPastDue(row.dueDate)) }));
  const apRows = bills.filter((row) => n(row.outstandingAmount) > 0 && row.status !== "DRAFT").map((row) => ({ ...row, overdueDays: daysPastDue(row.dueDate), aging: bucket(daysPastDue(row.dueDate)) }));
  ...
  return (
    <>
      <h2>Financial Reports</h2>
      <p className="small">Live management reports derived from posted journal lines and AR/AP documents.</p>
      ...
      <div className="grid">
        ...
        <div className="card"><div className="label">Balance Check</div><div className="value">{money(assets - liabilities - equity - netProfit)}</div></div>
      </div>
      ...
      <section className="panel table-wrap">
        ...
        <h4>Open Supplier Bills</h4>
        <table className="data-table"><thead><tr><th>Bill</th><th>Supplier</th><th>Project</th><th>Due</th><th>Days</th><th>Outstanding</th></tr></thead><tbody>{apRows.map((row) => <tr key={row.billId}><td>{row.billNumber}</td><td>{row.supplierId}</td><td>{row.projectId || "—"}</td><td>{row.dueDate || "—"}</td><td>{row.overdueDays}</td><td>{money(n(row.outstandingAmount))}</td></tr>)}</tbody></table>
      </section>
    </>
  );
}
