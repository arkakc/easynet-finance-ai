...
const money = (value: number, currency = "PGK") => new Intl.NumberFormat("en-PG", {
  style: "currency",
  currency,
...
export default function ReportsPage() {
  ...
  const exportCsv = () => {
    if (!data) return;
    const rows: Array<Array<string | number>> = [["Easynet Financial Statements"], ["Period", data.period.from, data.period.asOf], [], ["Profit & Loss"], ["Account", "Name", `Amount (${data.currency})`]];
        ...
        <>
          ...
          <section className="panel table-wrap">
            ...
            <table className="data-table"><thead><tr><th>Control</th><th>Ledger</th><th>Comparison</th><th>Difference</th><th>Status</th></tr></thead><tbody>
              ...
              <tr><td>AR control = customer subledger</td><td>{money(data.controls.receivables.glBalance, data.currency)}</td><td>{money(data.controls.receivables.subledgerBalance, data.currency)}</td><td>{money(data.controls.receivables.difference, data.currency)}</td><td><Status ok={data.controls.receivables.matched} /></td></tr>
              <tr><td>AP control = supplier subledger</td><td>{money(data.controls.payables.glBalance, data.currency)}</td><td>{money(data.controls.payables.subledgerBalance, data.currency)}</td><td>{money(data.controls.payables.difference, data.currency)}</td><td><Status ok={data.controls.payables.matched} /></td></tr>
            </tbody></table>
          </section>
          ...
          {(["receivables", "payables"] as const).map((kind) => {
            ...
            return <section className="panel table-wrap" key={kind}>
              ...
              <table className="data-table"><thead><tr><th>Document</th><th>{receivable ? "Customer" : "Supplier"}</th><th>Due</th><th>Days</th><th>Bucket</th><th>Outstanding</th></tr></thead><tbody>
                {aging.rows.map((row) => <tr key={row.id}><td><Link href={`/transactions/${receivable ? "invoice" : "supplierBill"}/${encodeURIComponent(row.id)}`}><strong>{row.code}</strong></Link></td><td>{row.partyCode} — {row.partyName}</td><td>{row.dueDate || "—"}</td><td>{row.overdueDays}</td><td>{row.bucket}</td><td>{money(row.outstanding, data.currency)}{row.transactionCurrency && row.transactionCurrency !== data.currency && row.transactionOutstanding !== undefined ? <><br/><span className="small">Original: {money(row.transactionOutstanding, row.transactionCurrency)}</span></> : null}</td></tr>)}
